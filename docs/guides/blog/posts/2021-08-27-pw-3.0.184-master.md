...
Updated the `$pages->getByPath()` method to support new options such as allowing URL segments to appear in given path, disabling partial path matching, and supporting paths that might have a root subdirectory url in it. [Details](../../../full/wire/core/Pages/method-getbypath.md)
...
New `$pages->findJoin('selector', [ fields ]);` method finds pages and autojoins given field names. [Details](../../../full/wire/core/Pages/method-findjoin.md)

New `$pages->findRaw('selector', [ fields ]);` method finds pages and returns raw field values from them in a PHP array. [Details](../../../full/wire/core/Pages/method-findraw.md)

New `$pages->getRaw('selector', [ fields ]);` method returns single page and requested raw field values. [Details](../../../full/wire/core/Pages/method-getraw.md)

New `$pages->getFresh('selector');` method returns a fresh, non-cached copy of the page from the database. [Details](../../../full/wire/core/Pages/method-getfresh.md)

New `$page->setUnformatted('field', $value);` method that temporarily turns off output formatting (if it was on), sets a known unformatted value, then turns output formatting back on (if it was previously on). [Details](../../../full/wire/core/Page/method-setunformatted.md)
...
New `$files->exists('/path/to/file', $options);` method for checking if a file exists with optional $options argument that can be one or more of space-separated words in a string to also validate that it matches: 'readable', 'writable', 'file', 'link', 'dir'. [Details](../../../full/wire/core/WireFileTools/method-exists.md)

New `$files->fileGetContents('/path/to/file/);` method that returns the entire contents of given file. Abstracted version of PHP's file_get_contents(). [Details](../../../full/wire/core/WireFileTools/method-filegetcontents.md)

New `$files->renameCopy($old, $new);` method that enables file rename by first copying files to destination and then deleting source files. [Details](../../../full/wire/core/WireFileTools/method-renamecopy.md)
...
Added support for starting/stopping debug mode DB query logging, enabling you to focus in on logging queries in between specific calls. See the boolean value option for the $sql argument of the `$database->queryLog()` method. [Details](../../../full/wire/core/WireDatabasePDO/method-querylog.md)

New `$database->getColumns('table', $options);` method returns an array of all columns in a table, with options. [Details](../../../full/wire/core/WireDatabasePDO/method-getcolumns.md)

New `$database->getIndexes('table', $options);` method returns all indexes from given table, with options. [Details](../../../full/wire/core/WireDatabasePDO/method-getindexes.md)

New `$database->indexExists('table', 'index');` method returns whether or not the given table has the given index. [Details](../../../full/wire/core/WireDatabasePDO/method-indexexists.md)

New `$database->getPrimaryKey('table');` method returns column(s) or info for given table’s primary key/index. [Details](../../../full/wire/core/WireDatabasePDO/method-getprimarykey.md)

New `$database->getTime();` method returns current ISO-8601 time string according to database server.

New `$database->getVersion();` method returns the database version string. [Details](../../../full/wire/core/WireDatabasePDO/method-getversion.md)

New `$database->getRegexEngine();` returns the type of regex engine the database uses. [Details](../../../full/wire/core/WireDatabasePDO/method-getregexengine.md)


### Sanitizer

Improvements have been made to the [$sanitizer](../../../full/wire/core/Sanitizer/index.md) array methods so that they can now support associative arrays with a ‘keySanitizer’ option that lets you specify which sanitizer method to clean up array indexes/keys.

The `$sanitizer->validateFile()` method was rewritten and improved, plus new ‘dryrun’ and ‘getArray’ options were added. [Details](../../../full/wire/core/Sanitizer/method-validatefile.md)

New `$sanitizer->textdomain('value');` method that returns value sanitized as language textdomain. [Details](../../../full/wire/core/Sanitizer/method-textdomain.md)

New `$sanitizer->selectorValueAdvanced();` method for sanitizing selector values using the advanced text search operator `#=`. [Details](../../../full/wire/core/Sanitizer/method-selectorvalueadvanced.md)


### Session

Added IPv6 support to `$session->getIP()`. [Details](../../../full/wire/core/Session/method-getip.md)
...
New `$session->location($url)` method that is like the redirect() method but implies a temporary (302) redirect. [Details](../../../full/wire/core/Session/method-location.md)

New `$session->hasLoginCookie();` method that returns true if a session login cookie is present. This only indicates the user was likely logged in at some point, and may not indicate an active login. [Details](../../../full/wire/core/Session/method-haslogincookie.md)
...
New `$inputfields->getByName('name');` method returns Inputfield having given name, or null if not found. Alias of getChildByName() method. $inputfields may be an InputfieldWrapper like a form or fieldset. [Details](../../../full/wire/core/InputfieldWrapper/method-getbyname.md)

New `$inputfields->getValueByName('name');` method that returns the value attribute from an Inputfield having given name. $inputfields may be an InputfieldWrapper like a form or fieldset. [Details](../../../full/wire/core/InputfieldWrapper/method-getvaluebyname.md)
...
New `$template->editUrl();` method that returns the URL to edit $template. [Details](../../../full/wire/core/Template/method-editurl.md)

New `$templates->add('name');` method that creates and returns a new template and fieldgroup and saves to the database. [Details](../../../full/wire/core/Templates/method-add.md)

New `$templates->rename($template, 'name');` method that renames a template, its fieldgroup and file (if writable). [Details](../../../full/wire/core/Templates/method-rename.md)
...
New `$field->getContexts();` method that returns all contexts that a Field is used in. [Details](../../../full/wire/core/Field/method-getcontexts.md)


### Input

New `$input->queryStringClean([ options ]);` method like the existing $input->queryString() method except that it enables you to get a cleaned up version according to rules you specify in an $options argument. [Details](../../../full/wire/core/WireInput/method-querystringclean.md)
...
New `$users->setAdminThemeByRole($adminTheme, $role);` method sets all users having given role to use given admin theme. [Details](../../../full/wire/core/Users/method-setadminthemebyrole.md)
...
The [WireHttp](../../../full/wire/core/WireHttp/index.md) class has been updated with a new sendStatusHeader() method to accompany the existing sendHeader() method. [Details](../../../full/wire/core/WireHttp/method-sendstatusheader.md)

New `$config->jsConfig();` method was added for setting or getting a value exclusive to ProcessWire’s javascript settings. [Details](../../../full/wire/core/Config/method-jsconfig.md)
