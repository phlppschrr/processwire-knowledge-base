...
In ProcessWire, the most basic representation of an item within a selector string follows the format `field=value` (see [Selectors documentation](/blog/categories/selectors/)). We use these throughout ProcessWire, but most commonly for finding pages via API calls like [$pages->find("selector")](../../../full/wire/core/Pages/method-___find.md).
