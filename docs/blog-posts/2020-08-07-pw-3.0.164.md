...
**New $pages->getByIDs() and $pages->getID() API methods** have also been added. [Details](../api-full/wire/core/Pages/index.md#pwapi-methods-retrieval)

**New Pages API hooks:** Pages::trashReady(), Pages::deleteBranchReady(), Pages::deletedBranch(). The deleteBranch hooks cover the case of recursive deletions where an entire branch of pages is deleted. For those cases, these hooks can be more useful than the regular deleteReady() and deleted() hooks because those two are called for every single page, rather than just for the initiating page. [Details](../api-full/wire/core/Pages/index.md#pwapi-methods-hooker)
...
**New find() and findOne() methods added to WireInputData class**, enabling `$input->post->find('…')`, `$input->get->find('…')`, etc., supporting both wildcards and regex patterns. [Details](../api-full/wire/core/WireInputData/method-find.md)


### Sanitizer API

**New $sanitizer methods:** [flatArray()](../api-full/wire/core/Sanitizer/method-flatarray.md), [wordsArray()](../api-full/wire/core/Sanitizer/method-wordsarray.md), and [word()](../api-full/wire/core/Sanitizer/method-word.md); plus a rewrite of the [selectorValue()](../api-full/wire/core/Sanitizer/method-selectorvalue.md) method with a lot of improvements and new options.
...
**New Template API methods:** [$template->allowNewPages()](../api-full/wire/core/Template/method-allownewpages.md) returns an up or down on whether the Template accepts new pages to be created with it; [$template->childTemplates()](../api-full/wire/core/Template/method-childtemplates.md) returns all Templates allowed to be used for its children; and [$template->parentTemplates()](../api-full/wire/core/Template/method-parenttemplates.md) returns all templates it allows for parents. [Details](https://processwire.com/blog/posts/pw-3.0.153/#template-updates)

**Upgrades to the core Debug class** adding support for PHP 7 hrtime() as well as the ability to [customize precision](../api-full/wire/core/Debug/index.md) and whether you prefer millisecond or floating point second timers. In addition a new [Debug::getAll()](../api-full/wire/core/Debug/index.md) method was added and this returns all of the active debug timers. [Details](../api-full/wire/core/Debug/index.md)
...
**Updated ProcessWire’s WireClassLoader** ($classLoader API var) to support autoloading by class prefix or suffix. [Details](../api-full/wire/core/WireClassLoader/index.md)
