...
**Added a new [uploadName()](../api-full/wire/core/Pagefile/method-uploadname.md) method/property to Pagefile/Pageimage objects that returns the original unsanitized filename as it was uploaded.** Applies only to files uploaded since this feature was added. Be aware the filename is unsanitized so be careful with it. File inputs now show this original filename if you hover the file icon, and image inputs show it if you hover the current filename.

**A new [$files->size($path)](../api-full/wire/core/WireFileTools/method-size.md) method was added which returns the total size of the given file or directory. **When given a directory, it returns the size of all files in that directory, recursively.


### API additions and improvements

**New [$sanitizer->htmlClass()](../api-full/wire/core/Sanitizer/method-htmlclass.md) and [$sanitizer->htmlClasses()](../api-full/wire/core/Sanitizer/method-htmlclasses.md) methods have been added. **These new methods sanitize HTML class attributes.

**The [WireHttp](../api-full/wire/core/WireHttp/index.md) class has new [delete](../api-full/wire/core/WireHttp/method-delete.md), [patch](../api-full/wire/core/WireHttp/method-patch.md) and [put](../api-full/wire/core/WireHttp/method-put.md) methods which correspond to http methods of the same name.** These might be used by web services in addition to the more common [get](../api-full/wire/core/WireHttp/method-get.md) and [post](../api-full/wire/core/WireHttp/method-post.md) methods. In prior versions of `WireHttp`, you could still use delete, patch and put methods, but had to use the [send](../api-full/wire/core/WireHttp/method-___send.md) method with the `$method` argument. Now that there are separate class methods for these http methods, it makes them a simpler to use and more clear in code. [Details](https://processwire.com/talk/topic/28681-weekly-update-%E2%80%93%C2%A030-june-2023/) and [More at weekly.pw](https://weekly.pw/issue/477/)
...
**New [WireNumberTools](../api-full/wire/core/WireNumberTools/index.md) class** and [uniqueNumber()](../api-full/wire/core/WireNumberTools/method-uniquenumber.md) method added to the core. This is a utility class for number functions. The `uniqueNumber()` method never returns the same number twice, for the life of the site. And the numbers it returns are not date/time-based. It's good for generating unique auto-incrementing numbers for any need, whether IDs or something else, and it avoids race conditions, ensuring there is no overlap even if they are generated at the same exact time. The new class adds several other useful methods as well. [Details](https://processwire.com/blog/posts/pw-3.0.213/#new-wirenumbertools-class)

**Improvements were made to ProcessWire's log tools [WireLog](../api-full/wire/core/WireLog/index.md) and [FileLog](../api-full/wire/core/FileLog/index.md),** with new [deleteAll()](../api-full/wire/core/WireLog/method-deleteall.md) and [pruneAll()](../api-full/wire/core/WireLog/method-pruneall.md) methods for deleting or pruning all log files at once.

**The [$cache](../api-full/wire/core/WireCache/index.md) API (WireCache) is now able to support other cache storage options via installed modules. **The first such option is [WireCacheFilesystem](https://processwire.com/modules/wire-cache-filesystem/) which makes ProcessWire store its cache data in files rather than in the database. We anticipate there will be Redis and Memcache options available at some point too. [Details](https://processwire.com/talk/topic/28521-weekly-update-%E2%80%93-19-may-2023/) and [More at weekly.pw](https://weekly.pw/issue/472/#latest-core-updates-processwire-3.0.218-and-new-wirecache-filesystem-module)
...
**New multi-language month and day names. **The [WireDateTime](../api-full/wire/core/WireDateTime/index.md) class (aka the `$datetime` API variable) has been updated to support multi-language month and day names. Now all month and days are translatable in the `WireDateTime` file (/wire/core/WireDateTime.php). So if you request any date in a format that uses month names or abbreviations, or day names or abbreviations, they now support multi-language, whether you requested it from the [wireDate()](../api-full/wire/core/FunctionsAPI/index.md) function or the `$datetime` API variable. [Details](https://processwire.com/talk/topic/28752-pw-30222-%E2%80%93%C2%A0core-updates/)
...
Improvements were made to ProcessWire's [MarkupPagerNav](https://processwire.com/api/ref/markup-pager-nav/) pagination module. [Details](https://github.com/processwire/processwire/pull/260)
