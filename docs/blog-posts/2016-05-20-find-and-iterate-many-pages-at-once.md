...
While these two methods may work exactly the same from the API perspective, they are fundamentally different in terms of what they do behind the scenes. For most page finding operations, you'll still want to use [$pages->find()](../api-full/wire/core/Pages/method-___find.md), because it's technically more efficient for most of the cases you would encounter in developing a website. But if you are dealing with potentially large quantities, and you don't want to place limits on the quantity of returned results, look no further than [$pages->findMany()](../api-full/wire/core/Pages/method-findmany.md).
...
Be sure to see the [documentation page for $pages->findMany()](../api-full/wire/core/Pages/method-findmany.md) in the 3.x API reference (which includes an example of it in action), and keep up-to-date with the [ProcessWire Weekly](http://weekly.pw) this weekend!
