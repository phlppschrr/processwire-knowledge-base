...
ProcessWire's file and image fields (represented by [Pagefile](../api-full/wire/core/Pagefile/index.md) and Pageimage objects) already have an uppercase `URL` property which always returns a cache-busting version of the file's URL. This is the case whether you've got ProCache installed or not. But in ProcessWire master 3.0.96 or newer, ProCache Buster updates that URL property to use ProCache's enhanced buster, rather than the one built-in to the core. Meaning, even the core file field API benefits from Buster:
