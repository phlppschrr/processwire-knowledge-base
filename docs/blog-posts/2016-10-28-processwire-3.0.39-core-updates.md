...
**The function calls are always in scope** Lets say you had a custom function you created where you need to access the [$page](../api-full/wire/core/Page/index.md) API variable. You would either have to pass that `$page` variable as an argument to the function, or you would have to access it from `wire('page')`. Now you can just access `page()` and it doesn't matter if $page is in scope or not.
