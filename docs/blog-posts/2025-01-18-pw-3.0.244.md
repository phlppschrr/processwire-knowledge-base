...
The [PagesVersions](../api-full/wire/modules/Pages/PagesVersions/PagesVersions/index.md) module was added to the core. This module provides a simple API for managing page versions, and it is also used by the [PagesVersionsPro](/talk/topic/29358-pw-30232-%E2%80%93%C2%A0core-updates-new-version-modules/) module (available to ProDevTools and ProDrafts users). A second version of PagesVersions also added support for partial save and restore, which provides the ability to save or restore some fields and not others. PagesVersions supports the majority of ProcessWire Fieldtypes, core, 3rd party and ProFields, even repeaters (nested too).


### Files and Images

Updated the [Pagefile](../api-full/wire/core/Pagefile/index.md), [Pagefiles](../api-full/wire/core/Pagefiles/index.md), [Pageimage](../api-full/wire/core/Pageimage/index.md), [Pageimages](../api-full/wire/core/Pageimages/index.md), [FieldtypeFile](/api/ref/fieldtype-file/) and [FieldtypeImage](/api/ref/fieldtype-image/) classes to support new [getFiles()](../api-full/wire/core/Pagefile/method-getfiles.md) methods that return all files connected with the field, whether originals, variations (i.e. different sizes, crops) or extras (i.e. WebP).
...
[OR-groups](/docs/selectors/#or-groups) support has been added to the [Selectors](../api-full/wire/core/Selectors/index.md) class so that it can also be used with non-page matching.
...
The [$database](../api-full/wire/core/WireDatabasePDO/index.md) API has been updated with a new `reset()` method. This [$database->reset()](../api-full/wire/core/WireDatabasePDO/method-reset.md) method closes then restarts the DB connection.

The [$database->execute($query)](../api-full/wire/core/WireDatabasePDO/method-execute.md) method now uses the above mentioned reset() method to retry a failed query due to loss of connection.


### $pages Hooks

New [$pages->moveReady()](../api-full/wire/core/Pages/method-___moveready.md), [restoreReady()](../api-full/wire/core/Pages/method-___restoreready.md), and [renameReady()](../api-full/wire/core/Pages/method-___renameready.md) hooks. These hooks are called immediately before each action named in the hook.
...
Added a new [$pages->cacher()->hasCache($id)](../api-full/wire/core/PagesLoaderCache/method-hascache.md) method that returns true or false as to whether or not a given page ID has already been cached in memory.

Added a new [$pages->request()->getPageInfo()](../api-full/wire/core/PagesRequest/method-getpageinfo.md) method. This returns a verbose associative array (just like [this one](../api-full/wire/core/PagesPathFinder/method-get.md)) about the page that was matched for the request URL, including all the details of how it was matched and what additional features are used in the URL.
...
Added a new [$page->cloneable()](../api-full/wire/core/Page/index.md#api-cloneable) method that returns true if the current user is allowed to clone the page. Or use `$page->cloneable(true)` to check if the user is allowed to clone the page and its children together.
...
The [$page->editUrl()](../api-full/wire/core/Page/method-editurl.md) method now supports a 'vars' (array) option that contains additional query string variables that it should bundle in to returned URL. For example: `$page->editUrl([ vars' => [' foo' => bar' ] ]);`
...
Updated the Page API for getting/setting multi-language values. The methods [$page->setLanguageValues()](../api-full/wire/core/Page/index.md#api-setLanguageValues) and [$page->getLanguageValues()](../api-full/wire/core/Page/index.md#api-getLanguageValues) were added for setting and getting values from multiple languages at once.

The methods [$page->setLanguageStatus()](../api-full/wire/core/Page/index.md#api-setLanguageStatus) and [$page->getLanguageStatus()](../api-full/wire/core/Page/index.md#api-getLanguageStatus) were added for setting/getting the active status of a language or multiple languages at once. This replaces more cryptic calls like `$page->status1234 = true` (where 1234 is language ID).

The methods [$page->setLanguageName()](../api-full/wire/core/Page/index.md#api-setLanguageName) and [$page->getLanguageName()](../api-full/wire/core/Page/index.md#api-getLanguageName) were added for setting/getting the name of a page in a particular language or languages. This replaces more cryptic calls like `$page->name1234 = 'page-name-in-spanish';`

Added new [setLanguageValue($language, $value)](../api-full/wire/core/Inputfield/index.md#api-setLanguageValue) and [getLanguageValue($language)](../api-full/wire/core/Inputfield/index.md#api-getLanguageValue) methods to the [Inputfield](../api-full/wire/core/Inputfield/index.md) class when [LanguageSupport](/docs/multi-language-support/) is installed. This provides a nicer API for getting/setting multi-language values with Inputfields. Previously you could only get/set by dealing with `value1234` type settings (where 1234 is language ID).

Several new methods have been added to the LanguageTranslator class: Note: `$t = $user->language->translator();`

- [$t->getTranslationInfo($textdomain, $text)](../api-full/wire/modules/LanguageSupport/LanguageTranslator/method-gettranslationinfo.md) returns verbose array of information about the translation, where it came from, etc.
- [$t->getTranslationOrFalse($textdomain, $text)](../api-full/wire/modules/LanguageSupport/LanguageTranslator/method-gettranslationorfalse.md) returns translation text if translated, or false if not (rather than default language value).
- [$t->findTranslations($text)](../api-full/wire/modules/LanguageSupport/LanguageTranslator/method-findtranslations.md) searches across all translated files to find all translations for given text.
- [$t->findTranslation($text)](../api-full/wire/modules/LanguageSupport/LanguageTranslator/method-findtranslation.md) searches across all translated files to find the first available translation for given text.


### Inputfields

New support for [Inputfield header actions](/talk/topic/29989-weekly-update-%E2%80%93-10-may-2024/) have been added to the Inputfield APIs both in Javascript and PHP, see [implementation details](../api-full/wire/core/Inputfield/method-addheaderaction.md).
...
Added new [getByField()](../api-full/wire/core/InputfieldWrapper/method-getbyfield.md) method to the [InputfieldWrapper](../api-full/wire/core/InputfieldWrapper/index.md) class. This method enables you to recursively find an Inputfield within the wrapper by specifying the Field (object, name, ID) that it represents. This is useful in cases where the Inputfield name may differ from the Field name that it represents, and you only know the field name.

Added new [getByProperty](../api-full/wire/core/InputfieldWrapper/method-getbyproperty.md) method to the [InputfieldWrapper](../api-full/wire/core/InputfieldWrapper/index.md) class. This method enables you to recursively find an Inputfield by any configuration property that it provides.
...
Several new methods were added to the [SelectableOptionArray](../api-full/wire/modules/Fieldtype/FieldtypeOptions/SelectableOptionArray/index.md) class, which represents [FieldtypeOptions](/api/ref/fieldtype-options/) field values (checkboxes, radios, selects, etc.). Now it is much simpler to get, add, and remove options from the selection by referring to the option ID, value or title. The added methods include:

- [getByID($id)](../api-full/wire/modules/Fieldtype/FieldtypeOptions/SelectableOptionArray/method-getbyid.md)
- [getByValue($value)](../api-full/wire/modules/Fieldtype/FieldtypeOptions/SelectableOptionArray/method-getbyvalue.md)
- [getByTitle($title)](../api-full/wire/modules/Fieldtype/FieldtypeOptions/SelectableOptionArray/method-getbytitle.md)
- [addByID($id)](../api-full/wire/modules/Fieldtype/FieldtypeOptions/SelectableOptionArray/method-addbyid.md)
- [addByValue($value)](../api-full/wire/modules/Fieldtype/FieldtypeOptions/SelectableOptionArray/method-addbyvalue.md)
- [addByTitle($title)](../api-full/wire/modules/Fieldtype/FieldtypeOptions/SelectableOptionArray/method-addbytitle.md)
- [removeByID($id)](../api-full/wire/modules/Fieldtype/FieldtypeOptions/SelectableOptionArray/method-removebyid.md)
- [removeByValue($value)](../api-full/wire/modules/Fieldtype/FieldtypeOptions/SelectableOptionArray/method-removebyvalue.md)
- [removeByTitle($title)](../api-full/wire/modules/Fieldtype/FieldtypeOptions/SelectableOptionArray/method-removebytitle.md)
...
New HTTP methods for OPTIONS, CONNECT and TRACE have been added to [WireHttp](../api-full/wire/core/WireHttp/index.md).
...
Added a new [$datetime->strtodate()](../api-full/wire/core/WireDateTime/method-strtodate.md) function which works like `strtotime($str, $format)` but returns a formatted date string rather than a timestamp.

Updated the [$datetime->strtotime()](../api-full/wire/core/WireDateTime/method-strtotime.md) function to accept an `inputFormat` option which lets you specify the format the given date string is in, for cases when it may not be recognized by PHP based purely on format. Plus a new `outputFormat` option which lets you specify the format it should return in rather than a timestamp (delegating to the new strtodate method).

We have added support for an experimental [$config->userOutputFormatting](../api-full/wire/core/Config/index.md#api-userOutputFormatting) setting and updated ProcessProfile module to support it.

[WireCacheDatabase](../api-full/wire/core/WireCacheDatabase/index.md) (the default handler for [$cache](../api-full/wire/core/WireCache/index.md)) has been updated to improve the efficiency of cache maintenance. This also corrects the issue of past caches with an `0000-00-00` expiration date that would [incorrectly] never expire.

The TemplateFile [halt()](../api-full/wire/core/TemplateFile/method-halt.md) method has been updated to optionally accept string argument to output before halt of template file rendering. For instance: `return $this->halt('<h1>Later dude!</h1>');` from a template file.
