...
The [$files->unzip()](../api-full/wire/core/WireFileTools/method-unzip.md) method was rewritten to add a lot of new options and features, as well as support the new [FileValidatorZip](/api/ref/file-validator-zip/) core module. In particular, note all the new settings available with the [$options](../api-full/wire/core/WireFileTools/method-unzip.md) argument.
...
In ProcessWire 3.0.255+ now you can use `$users->new()`, `$roles->new()`, and `$permissions->new()`, all of which return a new instance of the given Page. They are functionally similar to the existing [$pages->new()](../api-full/wire/core/Pages/method-___new.md) method, but specific to their types.
...
A new [InputfieldWrapper::removeByName()](../api-full/wire/core/InputfieldWrapper/method-removebyname.md) method has been added for removing an Inputfield from the form by name. It doesn't matter if the referred Inputfield is nested within others or not, this method will find it and remove it:
...
A new [$notices->getVisible()](../api-full/wire/core/Notices/method-getvisible.md) method was added that returns all [Notice](../api-full/wire/core/Notice/index.md) objects visible to current user. This prevents the need for filtering notices at output time. For example, if a notice has the [logOnly](../api-full/wire/core/Notice/index.md#pwapi-methods-constants) flag, then that means it is intended only for log, and thus would be considered not visible. But this method filters for [many other flags](../api-full/wire/core/Notice/index.md#pwapi-methods-constants) as well.
...
The [$notices](../api-full/wire/core/Notices/index.md) API variable also has two new methods added: [render()](../api-full/wire/core/Notices/method-___render.md) and [renderText()](../api-full/wire/core/Notices/method-___rendertext.md). Both are hookable, meaning they can have the arguments modified, return value modified, or be completely overridden if needed. The `render()` method uses the current [$adminTheme](../api-full/wire/core/AdminTheme/index.md) to render all currently visible notices. It accepts a single `$options` (array) argument where you can override any of the classes, icons, or markup used by the notices. The following example is equivalent to the previous example above:

```
echo $notices->render();
```

The [$notices->renderText()](../api-full/wire/core/Notices/method-___rendertext.md) method is similar to the `render()` method except that it outputs plain text only (no markup). It is intended for console usage of ProcessWire where you might be outputting to a command line console. It produces output like this:
...
A new [WireRandom::string()](../api-full/wire/core/WireRandom/method-string.md) method has been added to the [WireRandom](../api-full/wire/core/WireRandom/index.md) class. This method lets you specify that it should create a random string using specific characters. The first argument is the length (or 0 for random length) and the second argument is a string containing the characters that it is allowed to use.
...
See a list of all [Page class hookable methods](../api-full/wire/core/Page/index.md#pwapi-methods-hooker).


### New $pages->addReady() hookable method

The Pages class ([$pages](../api-full/wire/core/Pages/index.md) API var) also got a new hookable method: [Pages::addReady()](../api-full/wire/core/Pages/method-___addready.md). This method is called whenever a new page is about to be added and saved to the DB. You might hook into this to make some modification to the page before it is created. This accompanies the existing [Pages::added()](../api-full/wire/core/Pages/method-___added.md) hookable method, which is called after the page has been saved to the DB.
...
ProcessWire 3.0.255+ now includes a new [FileValidator module](../api-full/wire/core/FileValidatorModule/index.md) for validating ZIP files: [FileValidatorZip](/api/ref/file-validator-zip/). It automatically validates uploaded zip files. This prevents bad zip files or zip files with zillions of files from causing issues after upload, further enhancing safety.
...
With this new version of ProcessWire, our [API reference](../api-full/index.md) continues to improve a lot. Documentation was improved or added for roughly 50 core modules. You'll find this documentation both in our online API reference and as phpdoc in the core PHP files.
