...
Each named entry in the `$config->imageSizes` array can contain width and height, but it can also contain any other option accepted in the `$options` argument of the `$image->size()` call. Meaning, you can specify things like quality, cropping, sharpening, and [more](../api-full/wire/core/Pageimage/method-size.md). This example below demonstrates a few:
