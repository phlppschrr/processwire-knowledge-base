...
Timing how long something takes is an important part of debugging and profiling and can be immensely helpful in tracking down bottlenecks and identifying ways to improve the performance of your code. ProcessWire comes with a really simple tool to time how long something takes: the [Debug::timer() functions](../api-full/wire/core/Debug/index.md). Here's how you use them:
...
If you ever wonder why a particular URL/path resolves to a particular page, URL segments, language, pagination, etc., ProcessWire has a really helpful class for identifying all the details of it: the [$pages->getInfoByPath()](../api-full/wire/core/Pages/method-getinfobypath.md) method. It can be used like this:
...
How might you render your own notifications? They are stored in the [$notices](../api-full/wire/core/Notices/index.md) API variable and all you have to do is iterate them. In the example below, I'm translating ProcessWire notices to Uikit "uk-alert" notifications on the front-end:
