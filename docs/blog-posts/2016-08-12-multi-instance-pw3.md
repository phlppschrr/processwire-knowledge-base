...
While optional, we also recommend providing the URL to the installation as the second argument (preferably including the scheme and hostname). That way if you make any [$page->url()](../api-full/wire/core/Page/method-url.md) or [$page->httpUrl()](../api-full/wire/core/Page/method-httpurl.md) calls, it will know where to point them to:
