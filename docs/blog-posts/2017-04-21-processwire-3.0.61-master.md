...
- Add new [$pages->findIDs()](../api-full/wire/core/Pages/method-findids.md) method. Like $pages->find() except returns array of IDs rather than Page objects. This is a faster method to use when you only need to know the matching page IDs.
- Add new [$pages->sort()](../api-full/wire/core/Pages/method-___sort.md) method.
- Add new [$pages->insertBefore()](../api-full/wire/core/Pages/method-___insertbefore.md) and [$pages->insertAfter()](../api-full/wire/core/Pages/method-___insertafter.md) methods.
- Update [$page->editUrl(true)](../api-full/wire/core/Page/method-editurl.md) method to force return of edit URL with scheme and hostname.
- The beginnings of page import/export functions are included, but currently a work in progress.
- Add an [Inputfield::val()](../api-full/wire/core/Inputfield/method-val.md) method for consistency with jQuery. This method is a shortcut for getting or setting the value attribute of Inputfield objects
- Add [$languages->setLocale()](../api-full/wire/modules/LanguageSupport/Languages/method-setlocale.md) and [$languages->getLocale()](../api-full/wire/modules/LanguageSupport/Languages/method-getlocale.md) methods.
- Add new [Inputfield::hasField](../api-full/wire/core/Inputfield/index.md) property.
- Expand upon the [$page->url()](../api-full/wire/core/Page/method-url.md) method to include an $options argument that adds many new capabilities.
- Improvements to the region() aka wireRegion() function calls.
- Improvements to the [$pages->findMany()](../api-full/wire/core/Pages/method-findmany.md) method.
- Add new hook: [Inputfield::renderReadyHook ](../api-full/wire/core/Inputfield/method-___renderreadyhook.md)– called when before an Inputfield is rendered.
...
- Make each page's parent page lazy loading for Page objects, so that it doesn't load the parent Page until a [$page->parent()](../api-full/wire/core/Page/method-parent.md) call is requested.
