# RepeaterPage

Source: `wire/modules/Fieldtype/FieldtypeRepeater/RepeaterPage.php`

RepeaterPage represents an individual repeater page item

ProcessWire 3.x, Copyright 2023 by Ryan Cramer
https://processwire.com

Groups:
Group: [other](group-other.md)

Methods:
Method: [setForPage()](method-setforpage.md)
Method: [getForPage()](method-getforpage.md)
Method: [setForField()](method-setforfield.md)
Method: [getForField()](method-getforfield.md)
Method: [getForRoot()](method-getforroot.md)
Method: [getForFieldRoot()](method-getforfieldroot.md)
Method: [getForPageRoot()](method-getforpageroot.md)
Method: [get()](method-get.md)
Method: [getDepth()](method-getdepth.md)
Method: [setDepth()](method-setdepth.md)
Method: [isPublic()](method-ispublic.md)
Method: [isReady()](method-isready.md)
Method: [getAccessTemplate()](method-getaccesstemplate.md)
