...
- [`$fileContentTypes: array`](method-filecontenttypes.md) Array of extensions and the associated MIME type for each (for file output). See /wire/config.php for details and defaults.
- [`$fileCompilerOptions: array`](method-filecompileroptions.md) Array of options for FileCompiler class. See /wire/config.php for details and defaults.
