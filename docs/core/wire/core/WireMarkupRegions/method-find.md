...
- string $markup HTML document markup to perform the find in.
- array $options Optional options to modify default behavior: - `single` (bool): Return just the markup from the first matching region (default=false). - `verbose` (bool): Specify true to return array of verbose info detailing what was found (default=false). - `wrap` (bool): Include wrapping markup? Default is false for id or attribute matches, and true for class matches. - `max` (int): Maximum allowed regions to return (default=500). - `exact` (bool): Return region markup exactly as-is? (default=false). Specify true when using return values for replacement. - `leftover` (bool): Specify true if you want to return a "leftover" key in return value with leftover markup. - `type` (string|int): Optional type name to return for each region.
