...
- int $bytes Quantity in bytes
- bool|int|array $small Make returned string as small as possible? (default=false) - `true` (bool): Yes, make returned string as small as possible. - `1` (int): Same as `true` but with space between number and unit label. - Or optionally specify the $options argument here if you do not need the $small argument.
