# WireSaveableItems

Source: `wire/core/WireSaveableItems.php`

ProcessWire WireSaveableItems

Wire Data Access Object, provides reusable capability for loading, saving, creating, deleting,
and finding items of descending class-defined types.

Groups:
Group: [other](group-other.md)

Methods:
Method: [getAll()](method-getall.md)
Method: [makeBlankItem()](method-makeblankitem.md)
Method: [makeItem()](method-makeitem.md)
Method: [getTable()](method-gettable.md)
Method: [getSort()](method-getsort.md)
Method: [getLoadQuerySelectors()](method-getloadqueryselectors.md)
Method: [getLoadQuery()](method-getloadquery.md)
Method: [load()](method-___load.md) (hookable)
Method: [loadRowsReady()](method-loadrowsready.md)
Method: [initItem()](method-inititem.md)
Method: [saveItemKey()](method-saveitemkey.md)
Method: [save()](method-___save.md) (hookable)
Method: [delete()](method-___delete.md) (hookable)
Method: [clone()](method-___clone.md) (hookable)
Method: [find()](method-___find.md) (hookable)
Method: [get()](method-get.md)
Method: [has()](method-has.md)
Method: [__isset()](method-__isset.md)
Method: [encodeData()](method-encodedata.md)
Method: [decodeData()](method-decodedata.md)
Method: [useFuel()](method-usefuel.md)
Method: [saveReady()](method-___saveready.md) (hookable)
Method: [saveReady()](method-___saveready.md) (hookable)
Method: [deleteReady()](method-___deleteready.md) (hookable)
Method: [cloneReady()](method-___cloneready.md) (hookable)
Method: [renameReady()](method-___renameready.md) (hookable)
Method: [saved()](method-___saved.md) (hookable)
Method: [added()](method-___added.md) (hookable)
Method: [deleted()](method-___deleted.md) (hookable)
Method: [cloned()](method-___cloned.md) (hookable)
Method: [renamed()](method-___renamed.md) (hookable)
Method: [__invoke()](method-__invoke.md)
Method: [__invoke()](method-__invoke.md)
Method: [log()](method-log.md)
Method: [error()](method-error.md)
Method: [__debugInfo()](method-__debuginfo.md)
Method: [useLazy()](method-uselazy.md)
Method: [unsetLazy()](method-unsetlazy.md)
